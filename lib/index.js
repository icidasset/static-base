...
export function run() {
  ...
  return (pattern, dir, attr = {}) => {
    ...
    const wd = cleanPath(globParent(pattern), { start: true, end: true });
    const deps = { ...attr, pattern, wd, root };

    const files = list(pattern, deps);

    return pflow.apply(null, sequence(sequenceItems, deps))(files);
  };
}
...
function sequence(items, deps) {
  return items.map((item) => {
    return (rvaluePrevious) => {
      const formattedItem = item.contructor === Array ? item : [item];
      const fn = formattedItem[0];
      const fnArgs = [...formattedItem.slice(1), deps, rvaluePrevious];
